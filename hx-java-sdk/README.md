...
coinrui-java-api is a lightweight Java library for interacting with the [coinrui API](https://www.coinrui.com/restapipub.html), providing complete API coverage, and supporting synchronous and asynchronous requests, as well as event streaming using WebSockets.

## Features
* Support for synchronous and asynchronous REST requests to all [General](https://www.coinrui.com/restapipub.html#user-content-general-endpoints), [Market Data](https://www.coinrui.com/restapipub.html#user-content-market-data-endpoints), [Account](https://www.coinrui.com/restapipub.html#user-content-account-endpoints) endpoints, and [User](https://www.coinrui.com/restapipub.html#user-content-user-data-stream-endpoints) stream endpoints.
* Support for User Data, Trade, Kline, and Depth event streaming using [coinrui WebSocket API](https://www.coinrui.com/restapipub.html#wss-endpoint).
...
```
<dependency>
  <groupId>com.coinrui.api</groupId>
  <artifactId>coinrui-api-client</artifactId>
  <version>1.0.0</version>
</dependency>
```

Alternatively, you can clone this repository and run the [examples](https://github.com/joaopsilva/coinrui-java-api/tree/master/src/test/java/com/coinrui/api/examples).
...
1. [`coinruiApiRestClient`](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/main/java/com/coinrui/api/client/coinruiApiRestClient.java), a synchronous/blocking [coinrui API](https://www.coinrui.com/restapipub.html) client;
2. [`coinruiApiAsyncRestClient`](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/main/java/com/coinrui/api/client/coinruiApiAsyncRestClient.java), an asynchronous/non-blocking [coinrui API](https://www.coinrui.com/restapipub.html) client;
3. [`coinruiApiWebSocketClient`](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/main/java/com/coinrui/api/client/coinruiApiWebSocketClient.java), a data streaming client using [coinrui WebSocket API](https://www.coinrui.com/restapipub.html#wss-endpoint).

These can be instantiated through the corresponding factory method of [`coinruiApiClientFactory`](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/main/java/com/coinrui/api/client/coinruiApiClientFactory.java), by passing the [security parameters](https://www.coinrui.com/restapipub.html#user-content-endpoint-security-type) `API-KEY` and `SECRET`, which can be created at [https://www.coinrui.com/userCenter/createApi.html](https://www.coinrui.com/userCenter/createApi.html).

```java
coinruiApiClientFactory factory = coinruiApiClientFactory.newInstance("API-KEY", "SECRET");
coinruiApiRestClient client = factory.newRestClient();
```
...
```java
coinruiApiWebSocketClient client = coinruiApiClientFactory.newInstance().newWebSocketClient();
```
...
Each of the methods on `coinruiApiWebSocketClient`, which opens a new web socket, takes a `coinruiApiCallback`, which is
called for each event received from the coinrui servers. 

The `coinruiApiCallback` interface also has a `onFailure(Throwable)` method, which, optionally, can be implemented to 
receive notifications if the web-socket fails, e.g. disconnection.   

```java
client.onAggTradeEvent(symbol.toLowerCase(), new coinruiApiCallback<AggTradeEvent>() {
    @Override
...
Each of the methods on `coinruiApiWebSocketClient`, which opens a new web socket, also returns a `Closeable`.
...
Please see [DepthCacheExample.java](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/test/java/com/coinrui/api/examples/DepthCacheExample.java) for an implementation which uses the coinrui-java-api for maintaining a local depth cache for a symbol. In the same folder, you can also find how to do caching of account balances, aggregated trades, and klines/candlesticks.
...
To make an asynchronous request it is necessary to use the `coinruiApiAsyncRestClient`, and call the method with the same name as in the synchronous version, but passing a callback [`coinruiApiCallback`](https://github.com/joaopsilva/coinrui-java-api/blob/master/src/main/java/com/coinrui/api/client/coinruiApiCallback.java) that handles the response whenever it arrives.
...
Every API method can potentially throw an unchecked `coinruiApiException` which wraps the error message returned from the coinrui API, or an exception, in case the request never properly reached the server.

```java
try {
  client.getOrderBook("UNKNOWN", 10);
} catch (coinruiApiException e) {
...
### More examples
An extensive set of examples, covering most aspects of the API, can be found at https://github.com/joaopsilva/coinrui-java-api/tree/master/src/test/java/com/coinrui/api/examples.
